...
std::string generateTrianglePoints(std::default_random_engine &engine,
  ...
  while (dist(posX - posX2, posY - posY2) < epsilon) {
    posX2 = posDist(engine);
    posY2 = posDist(engine);
  }

  double side = dist(posX - posX2, posY - posY2);
...
void fillDirectoryWithFiles(fs::path root, int count, int triangleCount,
                            ...
                            int impostorFile) {
  int impostorLine = -1;
  for (int i = 0; i < count; ++i, impostorLine = -1) {
    std::string filename = root / ("file" + std::to_string(i + 1) + ".txt");
    std::ofstream file(filename);
    if (i == impostorFile) {
      impostorLine =
          std::uniform_int_distribution<int>(0, triangleCount - 1)(engine);
    }
    for (int j = 0; j < triangleCount; ++j) {
      if (j == impostorLine) {
        std::cout << "Impostor: " << root << " \t file: " << filename
                  << "\tline : " << j << std::endl;
      }
      file << generateTrianglePoints(engine, j == impostorLine) << "\n";
    }
  }
}
...
bool isTriangleValid(std::smatch match) {
